...
- "가격 기록" 섹션: 흰 카드 안 막대그래프(기존 price-chart 데이터 사용, 마지막 막대만 라임 `#BFDF74`, 나머지 `#E3EDF3`, 상단 가격 라벨, 하단 날짜 행).
- "후기" 섹션: **정적 UI만.** StatTile 4개(후기 0개/평균 별점 없음/재구매 의사 없음/내 후기 작성 전) + 작성 폼(별점·재구매 select, textarea, 다크 "후기 저장" 버튼 — 동작 없음, disabled 아님·클릭 시 no-op).
- `/products/[id]`(실시간 검색 상세)는 가격 기록 데이터가 없으면 해당 섹션 생략, 상단 카드와 후기 정적 섹션만 동일 스타일 적용.

### 관심상품 `/saved`
- eyebrow/타이틀/설명 + StatTile 4개(관심상품 n개/가격 하락/가격 상승/최근 확인 — 실데이터 계산 가능 범위, 불가하면 시안 문구).
- 가로형 리스트 카드(120px 이미지 / 본문 / 우측 버튼 3개: 가격 기록 보기 outline·구매하러 가기 라임·관심상품 해제 텍스트 danger). "가격 변동 없음" 그린 칩은 실데이터 변동 상태로 표기.
