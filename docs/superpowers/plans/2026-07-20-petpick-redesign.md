...
export default function PriceCard({ stats }: { stats: ProductPriceStats }) {
  ...
  return (
    <Link className="price-card" href={`/catalog/${stats.externalProductId}`}>
      ...
      {stats.category ? <Badge variant="category">{stats.category}</Badge> : null}
...
주의: `ProductPriceStats`에 `category` 필드가 없으면(`src/lib/price-stats.ts` 확인) 카테고리 Badge 줄은 생략한다 — 타입에 없는 필드 추가 금지.
...
**Interfaces:**
- Consumes: `Button`, `StatTile`, `SectionHeading`, `Reveal`, `PriceCard`, 기존 `fetchCatalogSummary/fetchTopDrops`, `categories`, `formatCheckedAt`

- [ ] **Step 1: page.tsx 마크업 교체**

주요 변경: 히어로 2열 그리드(좌 카피+검색 / 우 "오늘의 수집 현황" 카드), 급락 특가 4개(`fetchTopDrops(4)`), 카테고리 타일 버튼형, `CategoryTopProducts` 섹션 제거(import도 제거), 하단 2열 카드(병원=그린, 가이드=흰). 시안 문구 그대로:
...
export default async function HomePage() {
  const [summary, topDrops] = await Promise.all([fetchCatalogSummary(), fetchTopDrops(4)]);
...
참고: 시안의 카테고리 타일 "n개 추적" 카운트는 카테고리별 개수 쿼리가 없으므로 "둘러보기 →"로 대체(로직 추가 금지 원칙). `fetchCategoryTopDrops`/`CategoryTopProducts` import 제거. `CategoryTopProducts.tsx` 파일은 다른 사용처가 없으면 삭제.
...
```bash
git add src/app/page.tsx src/app/styles/_home.scss src/app/globals.scss
git commit -m "feat: 홈을 리디자인 시안 레이아웃으로 교체"
```

기존 `_base.scss`/`_catalog.scss`에 남은 `.home-hero`, `.home-stats`, `.home-links`, `.category-tiles` 구버전 블록이 있으면 이 커밋에서 삭제.
`CategoryTopProducts.tsx`를 삭제한 경우 `git add -A src/components`로 포함.
