...
1. 로드맵 항목(아래) 중 우선순위 있는 것부터 — 특히 "추적 상품이 계속 늘어나기만 함" 항목은 카탈로그가 커지기 전에 먼저 다루는 게 좋음.
2. CI가 `npm ci` 대신 `npm install`을 쓰는 것을 Linux 환경에서 lock 재생성 후 되돌리기 (급하지 않음).
...
- (없음) — `AGENTS.md`를 새 설계("펫픽", 카탈로그/가격추적)로 갱신 완료(2026-07-16). 새로 생기면 여기 기록.
